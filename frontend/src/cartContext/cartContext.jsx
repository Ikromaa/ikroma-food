import React, { useContext,createContext, useReducer, useEffect, useCallback } from 'react'
import axios from 'axios'

const CartContext = createContext();

// REDUCER HANDLING CART ACTIONS LIKE ADD,REMOVE,UPDATE QUANTITIY AND ITEM
...
// INTIALIZE CART FROM LOCAL STORAGE
const initializer = () => {
    try {
...
export const CartProvider = ({ children }) => {
    ...
    useEffect(() => {
        ...
        // if (!token) return
        axios.get('https://garnet-tsunamic-blotchily.ngrok-free.dev/api/cart', {
    ...
    const addToCart = useCallback(async (item, qty) => {
        ...
        const res = await axios.post(
            'https://garnet-tsunamic-blotchily.ngrok-free.dev/api/cart',
    ...
    const removeFromCart = useCallback(async _id => {
        const token = localStorage.getItem('authToken')
        await axios.delete(
            `https://garnet-tsunamic-blotchily.ngrok-free.dev/api/cart/${_id}`,
            {
                withCredentials: true,
                headers: { Authorization: `Bearer ${token}` }
            }
    ...
    const clearCart = useCallback(async () => {
        const token = localStorage.getItem('authToken')
        await axios.post(
            'https://garnet-tsunamic-blotchily.ngrok-free.dev/api/cart/clear',
            {},
            {
                withCredentials: true,
                headers: { Authorization: `Bearer ${token}` }
