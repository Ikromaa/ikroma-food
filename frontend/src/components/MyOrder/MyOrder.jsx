...
const MyOrder = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await axios.get('https://garnet-tsunamic-blotchily.ngrok-free.dev/api/orders', {
                ...
                const formattedOrders = response.data.map(order => ({
                    ...
                    createdAt: new Date(order.createdAt).toLocaleDateString('id-ID', { //INDONESIA
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: '2-digit',
    ...
    return (
        <div className=' min-h-screen bg-gradient-to-br from-[#1a120b] to-[#3e2b1d] py-12 px-4 sm:px-6 lg:px-8'>
            <div className=' mx-auto max-w-7xl'>
                <div className=' flex justify-between items-center mb-8'>
                    <Link to='/' className=' flex items-center gap-2 text-amber-400 hover:to-amber-300' >
                        <FiArrowLeft className='text-xl' />
                        <span className=' font-bold'>Back to Home</span>
                    </Link>
                    <div className=' flex items-center gap-4'>
                        <span className=' to-amber-400/70 text-sm'>
                            {user?.email}
                        </span>
                    </div>
                </div>
                                            ...
                                            transition-colors group'>
                                                    ...
                                                    <div className=' space-y-2'>
                                                        {order.items.map((item, index) => (
                                                            ...
                                                            className=' flex items-center gap-3 p-2 bg-[#3a2b2b]/50 rounded-lg'>
                                                                <img src={`https://garnet-tsunamic-blotchily.ngrok-free.dev${item.item.imageUrl}`} alt={item.item.name} 
                                                ...
                                                <td className=' p-4'>
                                                    <div className=' flex flex-col gap-2'>
                                                        ...
                                                        <div className={`${paymentStatus.color} px-3 py-1.5 rounded-lg
                                                        text-sm`}>
                                                            {paymentStatus.icon}
