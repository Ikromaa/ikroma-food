...
const API_URL = 'https://garnet-tsunamic-blotchily.ngrok-free.dev'
...
const CartPage = () => {
    ...
    return (
        ...
        to-[#3e2b1d]'>
                    ...
                    <div className=' grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
                        {cartItems.map(({ _id, item, quantity }) => (
                            ...
                            <div key={_id}
                                ...
                                border-amber-500 backdrop-blur-sm flex flex-col items-center gap-4
                                ...
                                transform hover:-translate-y-1 animate-fade-in'>
                                    ...
                                    rounded-lg transition-transform duration-300'
                                        onClick={() => setSelectedImage(buildImageUrl(item.imageUrl || item.image))}>
                                            <img src={buildImageUrl(item.imageUrl || item.image)} alt={item.name} className=' w-full h-full
                                            object-contain' />
                                    </div>
                                    ...
                                    <div className=' w-full text-center'>
                                        ...
                                        <p className=' text-amber-100/80 font-cinzel mt-1'>{formatRupiah(item.price)}</p>
                                    </div>
            ...
            {selectedImage && (
                <div className=' fixed inset-0 z-50 flex items-center justify-center bg-amber-900/40 bg-opacity-75 backdrop-blur-sm
                p-4 overflow-auto'
